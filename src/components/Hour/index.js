...
return (
  ...
  <Sidebar />
    <form className='Form'>
        ...
        <div className='abre'>
        <label>hora de apertura:</label><input type='text' id='apertura'/>
        </div>
        <div className='cierra'>
        <label>hora de cierre:</label><input type='text' id='cierre'/>
        </div>
        <input type="submit" className="submit" onClick={handleSubmit} />
        <div className='desaparezco hora' id='hora'><h3 id='h3'></h3><br/><button onClick={(e) => {
                e.preventDefault()
                  let aparecer = document.getElementById('hora')
                  aparecer.classList.add('desaparezco')
        }}>close</button></div>
        <div className="prueba-notificaciones">
        <button onClick={arepa
        }>Send Message</button>
        </div>
    </form>
    </>         
